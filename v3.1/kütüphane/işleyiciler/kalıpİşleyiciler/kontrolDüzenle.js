...
const hataAyıkla = hataKaydı("kontrolDüzenle");
 ...
 */
const kontrolDüzenle = (veri, geriCagirma) => {
  // Sadece GET metodunda çalışmalı
  if (veri.metot == "get") {
    // Araya sıkıştırılacak kalıp verileri oluşturuyoruz (_üstBilgi.html için)
    const kalıpVerisi = {
      "başlık.konu": "Kontrol - Düzenle",
      "vücut.sınıf": "kontrolDüzenle"
    };

    // Sayfa kalıbını alma
    sayfaKalıbınıAl("kontrolDüzenle", kalıpVerisi, (hata, dizgi) => {
      if (!hata && dizgi) {
