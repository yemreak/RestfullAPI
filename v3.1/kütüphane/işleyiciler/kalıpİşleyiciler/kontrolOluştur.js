...
const hataAyıkla = hataKaydı("kontrolOluştur");
 ...
 */
const kontrolOluştur = (veri, geriCagirma) => {
  // Sadece GET metodunda çalışmalı
  if (veri.metot == "get") {
    // Araya sıkıştırılacak kalıp verileri oluşturuyoruz (_üstBilgi.html için)
    const kalıpVerisi = {
      "başlık.konu": "Kontrol - Oluştur",
      "vücut.sınıf": "kontrolOluştur"
    };

    // Sayfa kalıbını alma
    sayfaKalıbınıAl("kontrolOluştur", kalıpVerisi, (hata, dizgi) => {
      if (!hata && dizgi) {
